...
DUNGEON_ENTRANCES = {
    # docs on how warp addresses work.
    # An address for a warp depends on what line the warp source is in and what address the warp source table is in.
    # For example, if a warp you want randomized is on the first line below the warp table function, you would shift that address by 2.
    # Each warp is 4 bytes, so if you want to go to the next line, you would need to add 4 each time making it so that you would need to shift the address by 6 in line 2.
    # The list keeps going on and on and once you get good at this, you'll understand what I'm talking about. 
    # Thank you Ishigh1 for helping me understand how warp addresses work and for allowing me to build a proper documentation for warps in ages/seasons AP.
    # Be sure to use GameboyAddress(0x?? (the bank number the warp address is in), 0x???? (the actual warp address)).address_in_rom() when initalizing warps in this randomizer.
